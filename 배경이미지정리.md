...
## **background-repeat**
```background-repeat```은 이미지를 반복시킬 것인지 아닐 것인지, 그리고 반복시킨다면 어떤 방식으로 반복시킬 것인지 정해주는 속성이다.   
```repeat, no-repeat``` 외에도 다양한 옵션이 있다.   
```css
/* 반복하지 않음 */
...
div {
  ...
  background-image: url("https://i.imgur.com/WxH5bez.png"); /*이미지 링크 연결*/
  margin: 30px;
}
```
